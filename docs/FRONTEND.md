...
| Fix API-Path | — | **Fix chamadas à API sem prefixo `/api` (404 em `/gestor/horarios`):** 6 páginas do gestor chamavam `adminGet/adminPost/adminPut/adminPatch/adminDelete('/gestor/...')` (sem `/api`) — o `fetch` ia à **rota de página** do Next.js em vez do catch-all proxy `/api/gestor/[...path]`, causando `404` (para `/gestor/horarios`, que não é página) ou erro de parse JSON (para as que são página, recebendo HTML). O erro reportado foi `GET /gestor/horarios 404`. **(1) `lib/api.ts`** — adicionada função `normalizarPath(path)`: se o path não começar com `/api/` nem for URL absoluta (`http(s)://`), prefixa com `/api`. Aplicada em todos os 5 helpers (`adminGet/Post/Put/Patch/Delete`). Isto corrige **todas** as chamadas erradas (presentes e futuras) de forma transparente — `adminGet('/gestor/horarios')` e `adminGet('/api/gestor/horarios')` resultam ambos no mesmo endpoint. **(2) Limpeza das 6 páginas** para usar `/api/gestor/...` explicitamente (consistência com as páginas que já estavam corretas — `/gestor/equipa`, `/gestor/ausencias`, `/gestor/relatorios`, `/gestor/configuracoes`, `/staff/ausencias`): `/gestor/pacientes`, `/gestor/equipa/horarios`, `/gestor/calendario-consultas`, `/gestor/consultas`, `/gestor/protocolos`, `/gestor/documentos`. Páginas afetadas (com GET/POST/PUT/PATCH/DELETE): pacientes (5 chamadas), equipa/horarios (6, incluindo disponibilidade), calendario-consultas (2 GET), consultas (10: 6 GET + 4 writes), protocolos (4), documentos (3). tsc ✓ + next build ✓ (31/31 páginas). |
| Rebranding UI | — | **Refatorização do rebranding Autocell→FisioFernandes na UI do gestor (4 tarefas):** **(1) Login redirect (já correto — verificação):** o `login/page.tsx` (linha 104) já usava `rotaPorRole(data.utilizador.role)` e `rotaPorRole` (auth.ts) já mapeia `admin → /admin`, `diretor_clinico`/`rececionista → /gestor`, `fisioterapeuta → /staff`. Sem alterações necessárias. **(2) Ocultar "Configurações" para não-admins:** `gestor-sidebar.tsx` — adicionada propriedade `roles?: Role[]` ao `NavItem` e leitura do role via `lerUtilizador()` (com `useEffect` + cleanup); o item "Configurações" passa a declarar `roles: ["admin"]` — só o Super Admin o vê. Os utilizadores da clínica (diretor_clinico, rececionista) já não veem o item. **(3) Erro JSON no calendário (causa real):** o `/gestor/calendario-consultas` JÁ estava correto (usa `/api/gestor/consultas` + `/api/gestor/equipa`). O erro `Unexpected token '<'` reportado provinha das páginas `/gestor/propriedades` (chamadas a `/api/gestor/smoobu/*`, `/api/gestor/checklists`, `/api/gestor/propriedades/default-checklist`) e `/gestor/configuracoes` (chamadas a `/api/gestor/smoobu/*`, `/api/gestor/webhooks`, `/api/gestor/configuracoes/forcar-*`) — todos endpoints inexistentes que devolviam HTML 404. Corrigido na tarefa 4 (abaixo). **(4) Refatorizar "Propriedades" → "Salas" e remover Smoobu:** **`/gestor/propriedades/page.tsx`** — reescrita completa (721→242 linhas, -636): textos visuais "Propriedades"→"Salas"/"Salas de Tratamento"; removido dropdown de apartamentos Smoobu (`carregarSmoobu`, `propriedadesSmoobu`), botão "Importar do Smoobu" (`handleImportarPropriedades`), campo `smoobu_id` (formulário + tabela + modal), botão "Checklist Padrão" (`handleAplicarChecklistPadrao`, endpoint `/default-checklist`), select de "Modelo de Checklist" (`carregarModelosChecklist`, `/api/gestor/checklists`), e avisos de geocoding (Nominatim — a morada da sala é livre). Mantido: CRUD (nome, morada, duração padrão, ativo), select de "Fisioterapeuta Preferencial". **`/gestor/configuracoes/page.tsx`** — reescrita (391→155 linhas): removido bloco "Ações Smoobu", modal "Logs de Sincronização Smoobu", campo "Smoobu API Key", botões de "Testes Manuais (Cron Jobs)" que apontavam para endpoints inexistentes. Mantido: dados da empresa (nome, NIF, morada, telefone, email) suportados pelo backend (`GET/PUT /api/gestor/configuracoes`). tsc ✓ + next build ✓ (31/31 páginas; `/gestor/propriedades` 6.26→3.94 kB, `/gestor/configuracoes` 3.53→3.7 kB). |
